...
namespace HJZBYSJ.DataBase
{
    ...
    public class GameUtil
    {
        ...
        public static bool TestConnection()
        {
            try
            {
                MySqlConnection con = new MySqlConnection("Data Source = localhost;Initial Catalog = wuziqi;User ID = root; Password = 1234");
                con.Open();
                return true;
            }
            catch(Exception ex)
        ...
        public static DataTable GetAllGame()
        {
            try
            {
                    ...
                    MySqlDataAdapter da = new MySqlDataAdapter("SELECT GameName FROM game", con);
        ...
        // 获得一条游戏存档记录
        public static bool GetAt(int gameName, ref Game game)
        {
        ...
        public static bool Add(Game a)
        {
            try
            {
                ...
                using (con)
                {
                    ...
                    MySqlCommand cmd = con.CreateCommand();
                    // 添加Quota表
                    string sql = "INSERT INTO game (GameName,GameID,Step, IsWin, CurrentColor, GameBoardXmlStr,GameWiner,GameModel)VALUES (?GameName,?GameID,?Step, ?IsWin, ?CurrentColor, ?GameBoardXmlStr, ?GameWiner, ?GameModel)";
                    cmd.CommandText = sql;

                    cmd.Parameters.AddWithValue("?GameName", a.gameName);
                    cmd.Parameters.AddWithValue("?GameID", a.gameID);
                    cmd.Parameters.AddWithValue("?Step", a.step);
                    cmd.Parameters.AddWithValue("?IsWin", a.isWin);
                    cmd.Parameters.AddWithValue("?CurrentColor", a.CurrentColor);
                    cmd.Parameters.AddWithValue("?GameBoardXmlStr", a.gameBoardXmlStr);
                    cmd.Parameters.AddWithValue("?GameWiner", a.Winer.Color);
                    cmd.Parameters.AddWithValue("?GameModel", a.thisGameModel);
        ...
        // 删除一条游戏存档
        public static bool Delete(int gameName)
        {
            try
            {
                MySqlConnection con = new MySqlConnection("Data Source = localhost;Initial Catalog = wuziqi;User ID = root; Password = 1234");
                ...
                using (con)
                {
                    ...
                    cmd.CommandText = "DELETE FROM game WHERE GameName = ?GameName";
                    cmd.Parameters.AddWithValue("?GameName", gameName);
                    cmd.ExecuteNonQuery();
